...
local ZScreenUnit = Class(SCREEN, function(self)
    ...
    self.middle_root = self:AddChild(WIDGET("z_side_middle"))
    self.middle_root:SetScaleMode(SCALEMODE_PROPORTIONAL)
    self.middle_root:SetHAnchor(ANCHOR_MIDDLE)
    self.middle_root:SetVAnchor(ANCHOR_MIDDLE)
    self.middle_root:SetMaxPropUpscale(MAX_HUD_SCALE)
    ...
    self.middle_root = self.middle_root:AddChild(WIDGET("real_z_side_middle"))
