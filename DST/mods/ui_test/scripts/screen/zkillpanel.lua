...
local ZSkillPanel = Class(Widget, function(self, festival_key, season, overrides)
    Widget._ctor(self, "ZSkillPanle")

	self.overrides = overrides or {}

    self.zskill_root = self:AddChild(Widget("zskill_root"))
	self.zskill_root:SetPosition(self.overrides.offset_x or 0, self.overrides.offset_y or 0)

	local grid = nil
	if self.overrides.quagmire_gridframe then
		grid = self.zskill_root:AddChild( self:_BuildZSkillExplorer(festival_key, season) )
		grid:SetPosition(-10,0)

		local boarder_scale = .73
		local grid_boarder = self.zskill_root:AddChild(Image("images/quagmire_recipebook.xml", "quagmire_recipe_line_long.tex"))
		grid_boarder:SetScale(boarder_scale, boarder_scale)
		grid_boarder:SetPosition(0, 204)
		grid_boarder = self.zskill_root:AddChild(Image("images/quagmire_recipebook.xml", "quagmire_recipe_line_long.tex"))
		grid_boarder:SetScale(boarder_scale, -boarder_scale)
		...
		if TheWorld ~= nil then
			local notice = self.zskill_root:AddChild(Text(CHATFONT, 20, "", self.overrides.primary_font_colour or UICOLOURS.HIGHLIGHT_GOLD))
	...
	if not self.overrides.no_title then
		local title = self.zskill_root:AddChild(Text(HEADERFONT, 28, "技能书", self.overrides.primary_font_colour or UICOLOURS.HIGHLIGHT_GOLD))
		title:SetPosition(0, 222)
	end

	-- local unlocked, total = EventAchievements:GetNumAchievementsUnlocked(festival_key, season)
    local unlocked, total = 0, 10

    local completed = self.zskill_root:AddChild(Text(HEADERFONT, 24, subfmt(STRINGS.UI.XPUTILS.XPPROGRESS, {num=unlocked, max=total}), self.overrides.primary_font_colour or UICOLOURS.HIGHLIGHT_GOLD))
	if not self.overrides.no_title then
...
function AchievementsPanel:_BuildZSkillExplorer(current_eventid, season)
    ...
    local function ScrollWidgetsCtor(context, index)
        local w = Widget("zskill-cell-".. index)
